...
function Sidebar({ view, setView, queueCount }) {
...
function App() {
  ...
  async function prepareArchive(files) {
    ...
    setReviewBusy(true);
    try {
      setRecycleReview(await apiReviewArchive(files.map(file => file.path), false));
    } catch (reviewError) {
      setReviewFiles(null);
      setError(reviewError.message);
    } finally {
  ...
  async function changeCleanup(value) {
    ...
    try {
      ...
      setRecycleReview(await (archive ? apiReviewArchive(reviewFiles.map(file => file.path), value) : apiReviewRecycle(reviewFiles.map(file => file.path), value)));
    } catch (reviewError) {
      setError(reviewError.message);
    } finally {
      setReviewBusy(false);
  ...
  async function moveSelected() {
    ...
    try {
      const archive = reviewMode === 'archive';
      const result = archive ? await apiArchive(reviewFiles.map(file => file.path), removeCatalogRows) : await apiRecycle(reviewFiles.map(file => file.path), removeCatalogRows);
      ...
      if (archive) {
        ...
        const catalogNote = result.catalog?.error ? ' Catalog DB cleanup failed: ' + result.catalog.error : result.catalog?.removed ? ' and removed ' + result.catalog.removed + ' Catalog DB entries.' : result.catalog?.backupPath ? ' Catalog DB backup created.' : '';
        setToast(failed ? 'Archived ' + moved + ' files; ' + failed + ' could not be moved.' + catalogNote : 'Archived ' + moved + ' conversation files.' + catalogNote);
  ...
  return <div className="app-shell"><WindowChrome /><div className="app-body"><Sidebar view={view} setView={setView} queueCount={selectedPaths.size} /><div className="main-shell"><Header data={data} onScan={scan} onCancel={cancelScan} scanning={scanning} scanStatus={scanStatus} view={view} />{error && <div className="error-banner"><Icon name="info" /><span>{error}</span><button onClick={() => setError('')}><Icon name="x" /></button></div>}<main className={'workspace ' + (browsingRoots ? '' : 'single')}>{content}</main><footer className="statusbar"><span><span className="status-dot" />Local only</span><span>{data ? data.stats.groupCount + ' sessions / ' + data.stats.fileCount + ' JSONL files' : 'Preparing scan'}</span></footer></div></div>{reviewFiles && <ReviewModal files={reviewFiles} safety={recycleReview} archive={reviewMode === 'archive'} onClose={() => { setReviewFiles(null); setRecycleReview(null); setRemoveCatalogRows(false); }} onConfirm={moveSelected} busy={moving} reviewBusy={reviewBusy} removeCatalogRows={removeCatalogRows} onCleanupChange={changeCleanup} />}{toast && <div className="toast"><Icon name="check" size={16} />{toast}</div>}</div>;
}

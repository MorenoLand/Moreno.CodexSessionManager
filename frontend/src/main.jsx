...
function WindowChrome() {
  ...
  return <div className="window-chrome" role="toolbar" aria-label="Window controls"><div className="window-drag-region" onDoubleClick={desktop ? toggleMaximise : undefined}><span className="window-app-icon"><Icon name="archive" size={16} /></span><strong>Session Shelf</strong></div>{desktop && <div className="window-controls"><button className="window-control" type="button" onClick={() => minimiseWindow().catch(() => {})} aria-label="Minimize" title="Minimize"><Icon name="minimize" size={15} /></button><button className="window-control" type="button" onClick={toggleMaximise} aria-label={maximised ? 'Restore' : 'Maximize'} title={maximised ? 'Restore' : 'Maximize'}><Icon name={maximised ? 'restore' : 'maximize'} size={14} /></button><button className="window-control close" type="button" onClick={() => closeWindow().catch(() => {})} aria-label="Close" title="Close"><Icon name="x" size={15} /></button></div>}</div>;
}
